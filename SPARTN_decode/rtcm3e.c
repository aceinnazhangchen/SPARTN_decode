...
{
    int prn = 0;
    //if (satsys(sat,&prn)!=_SYS_GLO_||rtcm->nav.geph[prn-1].sat!=sat) return -1;
	//return rtcm->nav.geph[prn-1].frq+7;
	if (satsys(sat, &prn) != _SYS_GLO_)
		return -1;
	else
		return get_glo_frq(prn) + 7;
}
#if 0
                        ...
                        int *pr21, int *ppr2, int *lock2, int *cnr2)
{
    ...
    *pr1=*amb=0;
    ...
    lt1=locktime(obs->time,rtcm->lltime[data->sat-1]  ,data->LLI[0]);
    lt2=locktime(obs->time,rtcm->lltime[data->sat-1]+1,data->LLI[1]);
...
/* encode rtcm header --------------------------------------------------------*/
static int encode_head(int type, rtcm_t *rtcm, obs_t *obs, int sys, int sync, int nsat)
{
    double tow;
    int i=24,week,epoch;
    ...
    setbitu(rtcm->buff,i,12,obs->staid); i+=12; /* ref station id */
...
static int encode_type1001(rtcm_t *rtcm, int sync)
{
    ...
    for (j=0;j<rtcm->obs.n&&nsat<MAXOBS;j++) {
        ...
        if (!(sys&(_SYS_GPS_|_SYS_SBS_))) continue;
        nsat++;
    }
    /* encode header */
    i=encode_head(1001,rtcm,_SYS_GPS_,sync,nsat);
    
    for (j=0;j<rtcm->obs.n&&nsat<MAXOBS;j++) {
        sys=satsys(rtcm->obs.data[j].sat,&prn);
        if (!(sys&(_SYS_GPS_|_SYS_SBS_))) continue;
        
        if (sys==_SYS_SBS_) prn-=80; /* 40-58: sbas 120-138 */
...
static int encode_type1002(rtcm_t *rtcm, int sync)
{
    ...
    /* encode header */
    i=encode_head(1002,rtcm,_SYS_GPS_,sync,nsat);
...
static int encode_type1003(rtcm_t *rtcm, int sync)
{
    ...
    /* encode header */
    i=encode_head(1003,rtcm,_SYS_GPS_,sync,nsat);
...
static int encode_type1004(rtcm_t *rtcm, int sync)
{
    ...
    /* encode header */
    i=encode_head(1004,rtcm,_SYS_GPS_,sync,nsat);
...
static int encode_type1005(rtcm_t *rtcm, obs_t *obs, int sync)
{
    double *p= obs->pos;
    int i=24;
...
static int encode_type1009(rtcm_t *rtcm, int sync)
{
    ...
    for (j=0;j<rtcm->obs.n&&nsat<MAXOBS;j++) {
        ...
        if (satsys(sat,&prn)!=_SYS_GLO_) continue;
        nsat++;
    }
    /* encode header */
    i=encode_head(1009,rtcm,_SYS_GLO_,sync,nsat);
...
static int encode_type1010(rtcm_t *rtcm, int sync)
{
    ...
    /* encode header */
    i=encode_head(1010,rtcm,_SYS_GLO_,sync,nsat);
...
static int encode_type1011(rtcm_t *rtcm, int sync)
{
    ...
    /* encode header */
    i=encode_head(1011,rtcm,_SYS_GLO_,sync,nsat);
...
static int encode_type1012(rtcm_t *rtcm, int sync)
{
    ...
    /* encode header */
    i=encode_head(1012,rtcm,_SYS_GLO_,sync,nsat);
...
static int encode_type1019(rtcm_t *rtcm, int sync)
{
    ...
    if (satsys(rtcm->ephsat,&prn)!=_SYS_GPS_) return 0;
...
static int encode_type1020(rtcm_t *rtcm, int sync)
{
    ...
    if (satsys(rtcm->ephsat,&prn)!=_SYS_GLO_) return 0;
...
static int encode_type1044(rtcm_t *rtcm, int sync)
{
    ...
    if (satsys(rtcm->ephsat,&prn)!=_SYS_QZS_) return 0;
    eph=rtcm->nav.eph+rtcm->ephsat-1;
    if (eph->sat!=rtcm->ephsat) return 0;
    week=eph->week%1024;
    toe  =ROUND(eph->toes/16.0);
...
static int encode_type1045(rtcm_t *rtcm, int sync)
{
    ...
    if (satsys(rtcm->ephsat,&prn)!=_SYS_GAL_) return 0;
    eph=rtcm->nav.eph+rtcm->ephsat-1;
    if (eph->sat!=rtcm->ephsat) return 0;
...
static int encode_type1042(rtcm_t *rtcm, int sync)
{
    ...
    if (satsys(rtcm->ephsat,&prn)!=_SYS_BDS_) return 0;
    eph=rtcm->nav.eph+rtcm->ephsat-1;
    if (eph->sat!=rtcm->ephsat) return 0;
    week =eph->week%8192;
                           ...
                           int solid)
{
    ...
    switch (sys) {
        case _SYS_GPS_: msgno=1056+type; break;
        case _SYS_GLO_: msgno=1062+type; break;
        case _SYS_GAL_: msgno=1239+type; break;
        case _SYS_QZS_: msgno=1245+type; break;
        case _SYS_BDS_: msgno=1257+type; break;
        case _SYS_SBS_: msgno=1251+type; break;
        default: return 0;
    }
...
static int encode_ssr1(rtcm_t *rtcm, int sys, int sync)
{
    ...
    switch (sys) {
        case _SYS_GPS_: np=6; ni= 8; nj= 0; offp=  0; break;
        case _SYS_GLO_: np=5; ni= 8; nj= 0; offp=  0; break;
        case _SYS_GAL_: np=6; ni=10; nj= 0; offp=  0; break;
        case _SYS_QZS_: np=4; ni= 8; nj= 0; offp=192; break;
        case _SYS_BDS_: np=6; ni=10; nj=24; offp=  1; break;
        case _SYS_SBS_: np=6; ni= 9; nj=24; offp=120; break;
        default: return 0;
    }
...
static int encode_ssr2(rtcm_t *rtcm, int sys, int sync)
{
    ...
    switch (sys) {
        case _SYS_GPS_: np=6; offp=  0; break;
        case _SYS_GLO_: np=5; offp=  0; break;
        case _SYS_GAL_: np=6; offp=  0; break;
        case _SYS_QZS_: np=4; offp=192; break;
        case _SYS_BDS_: np=6; offp=  1; break;
        case _SYS_SBS_: np=6; offp=120; break;
        default: return 0;
    }
    /* number of satellites */
    for (j=nsat=0;j<MAXSAT;j++) {
...
static int encode_ssr3(rtcm_t *rtcm, int sys, int sync)
{
    ...
    switch (sys) {
        case _SYS_GPS_: np=6; offp=  0; codes=codes_gps; ncode=17; break;
        case _SYS_GLO_: np=5; offp=  0; codes=codes_glo; ncode= 4; break;
        case _SYS_GAL_: np=6; offp=  0; codes=codes_gal; ncode=19; break;
        case _SYS_QZS_: np=4; offp=192; codes=codes_qzs; ncode=13; break;
        case _SYS_BDS_: np=6; offp=  1; codes=codes_bds; ncode= 9; break;
        case _SYS_SBS_: np=6; offp=120; codes=codes_sbs; ncode= 4; break;
        default: return 0;
    }
    /* number of satellites */
    for (j=nsat=0;j<MAXSAT;j++) {
...
static int to_satid(int sys, int sat)
{
    ...
    if      (sys==_SYS_QZS_) prn-=MINPRNQZS-1;
    else if (sys==_SYS_SBS_) prn-=MINPRNSBS-1;
...
static int to_sigid(int sys, unsigned char code, int *freq)
{
	...
	code2obs(sys, code, freq);
	sig = obscodes[code];
    if (!*(sig)) return 0;
	//if (!*(sig = code2obs(sys, code, freq))) return 0;
    
    switch (sys) {
        case _SYS_GPS_: msm_sig=msm_sig_gps; break;
        case _SYS_GLO_: msm_sig=msm_sig_glo; break;
        case _SYS_GAL_: msm_sig=msm_sig_gal; break;
        case _SYS_QZS_: msm_sig=msm_sig_qzs; break;
        case _SYS_SBS_: msm_sig=msm_sig_sbs; break;
        case _SYS_BDS_: msm_sig=msm_sig_cmp; break;
        default: return 0;
    }
...
static void gen_msm_index(rtcm_t *rtcm, obs_t *obs, int sys, int *nsat, int *nsig,
                          ...
                          unsigned char *sig_ind, unsigned char *cell_ind)
{
    ...
    *nsat=*nsig=*ncell=0;
    ...
    for (i=0;i<(int)obs->n;i++) {
        if (!(sat=to_satid(sys,obs->data[i].sat))) continue;
        
        for (j=0;j<NFREQ+NEXOBS;j++) {
            if (!(sig=to_sigid(sys,obs->data[i].code[j],&f))) continue;
...
static void gen_msm_sat(rtcm_t *rtcm, obs_t *obs, int sys, int nsat,
                        ...
                        double *rrate, unsigned char *info)
{
    ...
    for (i=0;i<(int)obs->n;i++) {
        ...
        for (j=0;j<NFREQ+NEXOBS;j++) {
            ...
            k=sat_ind[sat-1]-1;
            lambda=satwavelen(data->sat,f-1);
            ...
            if (info) info[k]=sys!=_SYS_GLO_?0:(fcn<0?15:fcn);
        }
    }
}
/* generate msm signal data fields -------------------------------------------*/
static void gen_msm_sig(rtcm_t *rtcm, obs_t *obs, int sys, int nsat, int nsig, int ncell,
                        ...
                        float *cnr)
{
    ...
    for (i=0;i<(int)obs->n;i++) {
        ...
        for (j=0;j<NFREQ+NEXOBS;j++) {
            ...
            lt=locktime_d(obs->time,rtcm->lltime[data->sat-1]+j,LLI);
...
/* encode msm header ---------------------------------------------------------*/
static int encode_msm_head(int type, rtcm_t *rtcm, obs_t *obs, int sys, int sync, int *nsat,
                           ...
                           float *cnr)
{
    ...
    switch (sys) {
        case _SYS_GPS_: type+=1070; break;
        case _SYS_GLO_: type+=1080; break;
        case _SYS_GAL_: type+=1090; break;
        case _SYS_QZS_: type+=1110; break;
        case _SYS_SBS_: type+=1100; break;
        case _SYS_BDS_: type+=1120; break;
        default: return 0;
    }
    /* generate msm satellite, signal and cell index */
    gen_msm_index(rtcm,obs,sys,nsat,&nsig,ncell,sat_ind,sig_ind,cell_ind);
    ...
    setbitu(rtcm->buff,i,12,obs->staid ); i+=12; /* reference station id */
    ...
    /* generate msm satellite data fields */
    gen_msm_sat(rtcm,obs,sys,*nsat,sat_ind,rrng,rrate,info);
    
    /* generate msm signal data fields */
    gen_msm_sig(rtcm,obs,sys,*nsat,nsig,*ncell,sat_ind,sig_ind,cell_ind,rrng,rrate,
...
static int encode_msm1(rtcm_t *rtcm, obs_t* obs, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(1,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,NULL,psrng,
                            NULL,NULL,NULL,NULL,NULL))) {
        return 0;
    }
...
static int encode_msm2(rtcm_t *rtcm, obs_t *obs, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(2,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,NULL,NULL,
...
static int encode_msm3(rtcm_t *rtcm, obs_t* obs, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(3,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,NULL,psrng,
                            phrng,NULL,lock,half,NULL))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm4(rtcm_t *rtcm, obs_t* obs, int sys, int sync)
{
    double rrng[64],rrate[64],psrng[64],phrng[64],lock[64];
    float cnr[64];
    unsigned char half[64];
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(4,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,NULL,psrng,
...
static int encode_msm5(rtcm_t *rtcm, obs_t* obs, int sys, int sync)
{
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(5,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,info,psrng,
...
static int encode_msm6(rtcm_t *rtcm, obs_t* obs, int sys, int sync)
{
    double rrng[64],rrate[64],psrng[64],phrng[64],lock[64];
    float cnr[64];
    unsigned char half[64];
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(6,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,NULL,psrng,
                            phrng,NULL,lock,half,cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
static int encode_msm7(rtcm_t *rtcm, obs_t* obs, int sys, int sync)
{
    double rrng[64],rrate[64],psrng[64],phrng[64],rate[64],lock[64];
    float cnr[64];
    unsigned char info[64],half[64];
    ...
    /* encode msm header */
    if (!(i=encode_msm_head(7,rtcm,obs,sys,sync,&nsat,&ncell,rrng,rrate,info,psrng,
                            phrng,rate,lock,half,cnr))) {
        return 0;
    }
    /* encode msm satellite data */
...
extern int encode_rtcm3(rtcm_t *rtcm, obs_t* obs, int type, int sync)
{
    int ret=0;
    ...
    switch (type) {
        ...
        case 1005: ret=encode_type1005(rtcm,obs,sync);     break;
        ...
        //case 1057: ret=encode_ssr1(rtcm,_SYS_GPS_,sync); break;
        //case 1058: ret=encode_ssr2(rtcm,_SYS_GPS_,sync); break;
        //case 1059: ret=encode_ssr3(rtcm,_SYS_GPS_,sync); break;
        //case 1060: ret=encode_ssr4(rtcm,_SYS_GPS_,sync); break;
        //case 1061: ret=encode_ssr5(rtcm,_SYS_GPS_,sync); break;
        //case 1062: ret=encode_ssr6(rtcm,_SYS_GPS_,sync); break;
        //case 1063: ret=encode_ssr1(rtcm,_SYS_GLO_,sync); break;
        //case 1064: ret=encode_ssr2(rtcm,_SYS_GLO_,sync); break;
        //case 1065: ret=encode_ssr3(rtcm,_SYS_GLO_,sync); break;
        //case 1066: ret=encode_ssr4(rtcm,_SYS_GLO_,sync); break;
        //case 1067: ret=encode_ssr5(rtcm,_SYS_GLO_,sync); break;
        //case 1068: ret=encode_ssr6(rtcm,_SYS_GLO_,sync); break;
        case 1071: ret=encode_msm1(rtcm,obs,_SYS_GPS_,sync); break;
        case 1072: ret=encode_msm2(rtcm,obs,_SYS_GPS_,sync); break;
        case 1073: ret=encode_msm3(rtcm,obs,_SYS_GPS_,sync); break;
        case 1074: ret=encode_msm4(rtcm,obs,_SYS_GPS_,sync); break;
        case 1075: ret=encode_msm5(rtcm,obs,_SYS_GPS_,sync); break;
        case 1076: ret=encode_msm6(rtcm,obs,_SYS_GPS_,sync); break;
        case 1077: ret=encode_msm7(rtcm,obs,_SYS_GPS_,sync); break;
        case 1081: ret=encode_msm1(rtcm,obs,_SYS_GLO_,sync); break;
        case 1082: ret=encode_msm2(rtcm,obs,_SYS_GLO_,sync); break;
        case 1083: ret=encode_msm3(rtcm,obs,_SYS_GLO_,sync); break;
        case 1084: ret=encode_msm4(rtcm,obs,_SYS_GLO_,sync); break;
        case 1085: ret=encode_msm5(rtcm,obs,_SYS_GLO_,sync); break;
        case 1086: ret=encode_msm6(rtcm,obs,_SYS_GLO_,sync); break;
        case 1087: ret=encode_msm7(rtcm,obs,_SYS_GLO_,sync); break;
        case 1091: ret=encode_msm1(rtcm,obs,_SYS_GAL_,sync); break;
        case 1092: ret=encode_msm2(rtcm,obs,_SYS_GAL_,sync); break;
        case 1093: ret=encode_msm3(rtcm,obs,_SYS_GAL_,sync); break;
        case 1094: ret=encode_msm4(rtcm,obs,_SYS_GAL_,sync); break;
        case 1095: ret=encode_msm5(rtcm,obs,_SYS_GAL_,sync); break;
        case 1096: ret=encode_msm6(rtcm,obs,_SYS_GAL_,sync); break;
        case 1097: ret=encode_msm7(rtcm,obs,_SYS_GAL_,sync); break;
        case 1101: ret=encode_msm1(rtcm,obs,_SYS_SBS_,sync); break;
        case 1102: ret=encode_msm2(rtcm,obs,_SYS_SBS_,sync); break;
        case 1103: ret=encode_msm3(rtcm,obs,_SYS_SBS_,sync); break;
        case 1104: ret=encode_msm4(rtcm,obs,_SYS_SBS_,sync); break;
        case 1105: ret=encode_msm5(rtcm,obs,_SYS_SBS_,sync); break;
        case 1106: ret=encode_msm6(rtcm,obs,_SYS_SBS_,sync); break;
        case 1107: ret=encode_msm7(rtcm,obs,_SYS_SBS_,sync); break;
        case 1111: ret=encode_msm1(rtcm,obs,_SYS_QZS_,sync); break;
        case 1112: ret=encode_msm2(rtcm,obs,_SYS_QZS_,sync); break;
        case 1113: ret=encode_msm3(rtcm,obs,_SYS_QZS_,sync); break;
        case 1114: ret=encode_msm4(rtcm,obs,_SYS_QZS_,sync); break;
        case 1115: ret=encode_msm5(rtcm,obs,_SYS_QZS_,sync); break;
        case 1116: ret=encode_msm6(rtcm,obs,_SYS_QZS_,sync); break;
        case 1117: ret=encode_msm7(rtcm,obs,_SYS_QZS_,sync); break;
        case 1121: ret=encode_msm1(rtcm,obs,_SYS_BDS_,sync); break;
        case 1122: ret=encode_msm2(rtcm,obs,_SYS_BDS_,sync); break;
        case 1123: ret=encode_msm3(rtcm,obs,_SYS_BDS_,sync); break;
        case 1124: ret=encode_msm4(rtcm,obs,_SYS_BDS_,sync); break;
        case 1125: ret=encode_msm5(rtcm,obs,_SYS_BDS_,sync); break;
        case 1126: ret=encode_msm6(rtcm,obs,_SYS_BDS_,sync); break;
        case 1127: ret=encode_msm7(rtcm,obs,_SYS_BDS_,sync); break;
        //case 1240: ret=encode_ssr1(rtcm,_SYS_GAL_,sync); break;
        //case 1241: ret=encode_ssr2(rtcm,_SYS_GAL_,sync); break;
        //case 1242: ret=encode_ssr3(rtcm,_SYS_GAL_,sync); break;
        //case 1243: ret=encode_ssr4(rtcm,_SYS_GAL_,sync); break;
        //case 1244: ret=encode_ssr5(rtcm,_SYS_GAL_,sync); break;
        //case 1245: ret=encode_ssr6(rtcm,_SYS_GAL_,sync); break;
        //case 1246: ret=encode_ssr1(rtcm,_SYS_QZS_,sync); break;
        //case 1247: ret=encode_ssr2(rtcm,_SYS_QZS_,sync); break;
        //case 1248: ret=encode_ssr3(rtcm,_SYS_QZS_,sync); break;
        //case 1249: ret=encode_ssr4(rtcm,_SYS_QZS_,sync); break;
        //case 1250: ret=encode_ssr5(rtcm,_SYS_QZS_,sync); break;
        //case 1251: ret=encode_ssr6(rtcm,_SYS_QZS_,sync); break;
        //case 1252: ret=encode_ssr1(rtcm,_SYS_SBS_,sync); break;
        //case 1253: ret=encode_ssr2(rtcm,_SYS_SBS_,sync); break;
        //case 1254: ret=encode_ssr3(rtcm,_SYS_SBS_,sync); break;
        //case 1255: ret=encode_ssr4(rtcm,_SYS_SBS_,sync); break;
        //case 1256: ret=encode_ssr5(rtcm,_SYS_SBS_,sync); break;
        //case 1257: ret=encode_ssr6(rtcm,_SYS_SBS_,sync); break;
        //case 1258: ret=encode_ssr1(rtcm,_SYS_BDS_,sync); break;
        //case 1259: ret=encode_ssr2(rtcm,_SYS_BDS_,sync); break;
        //case 1260: ret=encode_ssr3(rtcm,_SYS_BDS_,sync); break;
        //case 1261: ret=encode_ssr4(rtcm,_SYS_BDS_,sync); break;
        //case 1262: ret=encode_ssr5(rtcm,_SYS_BDS_,sync); break;
        //case 1263: ret=encode_ssr6(rtcm,_SYS_BDS_,sync); break;
    }
    if (ret>0) {
        type-=1000;
...
*-----------------------------------------------------------------------------*/
extern int gen_rtcm3(rtcm_t* rtcm, obs_t *obs, int type, int sync)
{
	unsigned int crc;
	int i = 0;
	...
	if (!encode_rtcm3(rtcm, obs, type, sync)) return 0;
